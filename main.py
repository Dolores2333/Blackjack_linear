...
from AVAF import calculate_Q_sa, MC_linear, QL_linear, TD_linear
...
def blackjack(config, args, instance_dir):
    ...
    # Define the counters
    if linear:
        w = np.random.uniform(low=0.0, high=1.0, size=3)
        update = update + '_linear'
    ...
    for k in tqdm(range(epochs)):
        ...
        while not game.terminal:
            ...
            # Make sure the Q table is restored before being used
            if linear:
                Q_sa = calculate_Q_sa(Q_sa, w)
        ...
        if update == 'MC_linear':
            w = MC_linear(reward_list, keys_by_players, N_s, N_sa, alpha, w)
        if update == 'QL_linear':
            w = QL_linear(reward_list, keys_by_players, N_s, N_sa, alpha, w)
        if update == 'TD_linear':
            w = TD_linear(reward_list, keys_by_players, N_s, N_sa, alpha, w)
